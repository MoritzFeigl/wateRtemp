...
ann_metaf <- function(catchment, data_inputs,
  ...
  for(run in c(1:ensemble_runs)){
    ...
    ANN_checkpoint <- callback_model_checkpoint(
      filepath = paste0(catchment, "/ANN/", model_name, "/checkpoints/", "ensemble_member", run, ".hdf5"),
    ...
    # Training plot ------------------------------------------------------------------------
    png(paste0(catchment, "/ANN/", model_name, "/training_metrics/plot_ensemble_member_", run, ".png"),
        width = 800, heigh = 600)
  ...
  network_evaluation <- function(new_run){
    ...
    new_model %>% load_model_weights_hdf5(
      file.path(paste0(catchment, "/ANN/",
                       model_name, "/checkpoints/", "ensemble_member",  new_run, ".hdf5"))
    )
  ...
  feather::write_feather(pred_results,
                         paste0(catchment, "/ANN/", model_name, "/",
                                "ensemble_prediction_results.feather"))
  ...
  if(model_subset < ensemble_runs & model_subset >= 1){
    ...
    txt <- capture.output(file.remove(paste0(catchment, "/ANN/", model_name,
                                             "/training_metrics/loss_ensemble_member_",
                                             model_to_delete, ".feather")))
    txt <- capture.output(file.remove(
      paste0(catchment, "/ANN/", model_name, "/training_metrics/plot_ensemble_member_",
             model_to_delete, ".png"))
    )

    cp_files <- list.files(paste0(catchment, "/ANN/", model_name, "/checkpoints/"))
    ...
    txt <- capture.output(
      file.remove(paste0(catchment, "/ANN/", model_name, "/checkpoints/", cp_to_delete))
    )
  }
  ...
  feather::write_feather(data.frame("predicted_values" = pred_results_mean_rescaled),
                         paste0(catchment, "/ANN/", model_name, "/predicted_values.feather"))
  # Model scores
