...
#'
#' @examples
ann_metaf_DataXGB <- function(catchment, data_inputs,
  ...
  if(!file.exists(paste0(catchment, "/ANN_DataXGB/", model_name))){
    dir.create(file.path(paste0(catchment, "/ANN_DataXGB/", model_name)))
  }
  if(!file.exists(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name))){
    dir.create(file.path(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name)))
  }
  if(!file.exists(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name,
                         "/checkpoints"))){
    dir.create(file.path(paste0(catchment, "/ANN_DataXGB/", model_name, "/",
  ...
  for(run in c(1:ensemble_runs)){
    ...
    model %>%
      ...
      compile(loss = 'mean_squared_error',
              optimizer = tf$train$AdamOptimizer())#optimizer_adam())
    # Create checkpoint callback
    ANN_checkpoint <- callback_model_checkpoint(
      filepath = paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name,
    ...
    png(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name,
    ...
    feather::write_feather(
      ...
      paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name,
             "/training_metrics/loss_ensemble_member_", run, ".feather")
    )
  }
  cat("\nTraining plots and loss values saved in",
      paste0('"', catchment, "/ANN_DataXGB/", model_name, "/", folder_name, '/training_metrics"\n'))
  ...
  if(model_subset < ensemble_runs & model_subset >= 1){
    ...
    txt <- capture.output(file.remove(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name,
                                             "/training_metrics/loss_ensemble_member_",
                                             model_to_delete, ".feather")))
    txt <- capture.output(file.remove(
      paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name, "/training_metrics/plot_ensemble_member_",
             model_to_delete, ".png"))
    )

    cp_files <- list.files(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name, "/checkpoints/"))
    ...
    txt <- capture.output(
      file.remove(paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name, "/checkpoints/", cp_to_delete))
    )
  }
  if(model_subset != 1){
  ...
  feather::write_feather(data.frame("predicted_values" = mean_pred_results_test*train_sd + train_mean),
                         paste0(catchment, "/ANN_DataXGB/", model_name, "/", folder_name, "/predicted_values.feather"))
  # Rscaling and scores
  ...
  run_time <- paste0(
    ...
    " minutes")
  # Saving model scores
  if("model_scores.csv" %in% list.files(paste0(catchment, "/ANN_DataXGB/"))){
    model_scores <- read.csv(paste0(catchment, "/ANN_DataXGB/model_scores.csv"))
    ...
    model_scores <- data.frame(user = user_name,
                               ...
                               "RMSE_test" = round(RMSE_test, 3),
                               "NSE_test" = round(NSE_test, 3))
    write.csv(model_scores, paste0(catchment, "/ANN_DataXGB/model_scores.csv"), row.names = FALSE)
  }
}
