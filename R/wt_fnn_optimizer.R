...
#'
#' @examples
wt_fnn_optimizer <- function(catchment,
                             data_inputs = NULL,
                             n_iter = 20,
  ...
  if(!initial_grid_from_model_scores | ini_grid_cal_flag) {
    ...
    grid <- data.frame(
      ...
      "dropout" = replicate(n = n_random,
                            sample(
                              x = seq(bounds_dropout[1], bounds_dropout[2], by = 0.01),
                              size = 1)),
  ...
  if(n_iter > 0){
    ...
    Bopt_fnn_model <- function(layers, units, dropout,  batch_size) {
      results <- wt_fnn(catchment = catchment,
                        data_inputs = data_inputs,
                        layers = layers,
                        units = units,
                        dropout = dropout,
    ...
    Bopt_fnn <- rBayesianOptimization::BayesianOptimization(
      Bopt_fnn_model,
      ...
      verbose = TRUE)
    all_model_results <- Bopt_fnn$History
  } else {
  ...
  cat("\n*** Save the optimization result in FNN folder ***")
  ...
  new_opt_results <- data.frame(date = performance$start_time,
                                data_inputs = data_inputs,
                                RMSE_val = round(performance$RMSE_val, 4),
                                NSE_val = performance$NSE_val,
                                RMSE_test = round(performance$RMSE_test, 4),
  ...
  if("optimization_results.csv" %in% list.files(paste0(catchment, "/FNN"))){
    opt_results <- read.csv(paste0(catchment, "/FNN/optimization_results.csv"))
    new_opt_results <- rbind(opt_results, new_opt_results)
  }
  write.csv(new_opt_results,
            paste0(catchment, "/FNN/optimization_results.csv"),
